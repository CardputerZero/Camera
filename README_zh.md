...
```bash
./package_deb.sh -DCAMERA_APP_VERSION=1.0.0
```
...
```text
dist/Camera_1.0.0_m5stack1_arm64.deb
```
...
```bash
dpkg-deb -c dist/Camera_1.0.0_m5stack1_arm64.deb
```
...
```bash
sudo apt install ./dist/Camera_1.0.0_m5stack1_arm64.deb
```

或者复制到目标板后安装：

```bash
scp dist/Camera_1.0.0_m5stack1_arm64.deb pi@pi:~/
ssh pi@pi 'sudo apt install ./Camera_1.0.0_m5stack1_arm64.deb'
```

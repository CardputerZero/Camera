...
```bash
./package_deb.sh -DCAMERA_APP_VERSION=0.3.8
```
...
```text
dist/Camera_0.3.8_m5stack1_arm64.deb
```
...
```bash
dpkg-deb -c dist/Camera_0.3.8_m5stack1_arm64.deb
```
...
```bash
sudo apt install ./dist/Camera_0.3.8_m5stack1_arm64.deb
```

或者复制到目标板后安装：

```bash
scp dist/Camera_0.3.8_m5stack1_arm64.deb pi@pi:~/
ssh pi@pi 'sudo apt install ./Camera_0.3.8_m5stack1_arm64.deb'
```

...
void Logger::set_tag(const char* tag) { tag_ = tag ? tag : ""; }

const char* Logger::tag() { return tag_.c_str(); }

void Logger::set_color_mode(ColorMode mode) { color_mode_ = mode; }

void Logger::set_timestamp_enabled(bool enabled) { timestamp_enabled_ = enabled; }

bool Logger::timestamp_enabled() { return timestamp_enabled_; }

// conditional log
bool Logger::should_log(LogLevel level) { return level >= level_ && level_ != LogLevel::OFF; }
...
void Logger::log(LogLevel level, const std::string& msg, const char* file, int line) {
  ...
  if (should_use_color()) {
    printf("%s%s %s%s\n", level_to_color(level), prefix.c_str(), msg.c_str(), RESET);
  } else {
